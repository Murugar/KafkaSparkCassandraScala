...
startKafka := {
  (runMain in Compile).toTask(" com.iqmsoft.cassandra.kafka.spark.KafkaServer").value
}

val startCassandra = taskKey[Unit]("start-cassandra")

startCassandra := {
  (runMain in Compile).toTask(" com.iqmsoft.cassandra.kafka.spark.CassandraServer").value
}
